...
class WalkThroughFragmentFactory(
    ...
    override fun instantiate(classLoader: ClassLoader, className: String): Fragment {
        return when (className) {
            WTOneFragment::class.java.name -> WTOneFragment.newInstance(walkThroughItems[0])
            WTTwoFragment::class.java.name -> WTTwoFragment.newInstance(walkThroughItems[1])
            WTThreeFragment::class.java.name -> WTThreeFragment.newInstance(walkThroughItems[2])
